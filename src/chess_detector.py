...
def rotate_grid(row, col, orientation):
    if orientation == 'bottom': return row, col
    if orientation == 'top': return 7 - row, 7 - col
    if orientation == 'left': return col, row
    if orientation == 'right': return 7 - col, 7 - row
...
def generate_full_fen(board, orientation, active_player="w", en_passant="-", halfmove=0, fullmove=1, interactive=False):
    ...
    if piece_at("1", "e", "K"):
        if piece_at("1", "h", "R"): castling += "K"
        if piece_at("1", "a", "R"): castling += "Q"
    if piece_at("8", "e", "k"):
        if piece_at("8", "h", "r"): castling += "k"
        if piece_at("8", "a", "r"): castling += "q"
    if castling == "":
        castling = "-"
...
def visualize_grid(img, detections, H, orientation):
    ...
    for i in range(9):
        ax.plot([p[0] for p in grid_points[i]], [p[1] for p in grid_points[i]], color='white', linewidth=2)
    for j in range(9):
        ax.plot([grid_points[i][j][0] for i in range(9)], [grid_points[i][j][1] for i in range(9)], color='white', linewidth=2)
    ...
    for j in range(8):
        ...
        ax.text(mid_x + 20, mid_y + 30, square[0], ha='center', va='top', color='black', fontsize=12, fontweight='bold')
    ...
    for i in range(8):
        ...
        ax.text(mid_x - 25, mid_y + 20, square[1], ha='right', va='center', color='black', fontsize=12, fontweight='bold')
    ...
    for det in detections:
        ...
        ax.text(cx + 5, cy, board_coord, color='yellow', fontsize=10, fontweight='bold', bbox=dict(facecolor='black', alpha=0.6))
